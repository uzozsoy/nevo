"""Loss balancing utilities adapted from Meta's EnCodec project.

Attribution: portions of this module are derived from
https://github.com/facebookresearch/encodec under the MIT License.
See THIRD_PARTY_NOTICES.md for copyright, license, and citation details.
"""
...
def averager(beta: float = 1):
    ...
    # ---- add state for checkpoints ----
    _update.decay = beta
    _update.total = total
    _update.fix = fix

    def _state_dict():
        # store as plain Python types
        return {
            "decay": float(_update.decay),
            "total": dict(_update.total),
            "fix": dict(_update.fix),
        }

    def _load_state_dict(sd: tp.Dict[str, tp.Any]):
        nonlocal total, fix
        # restore decay
        beta_new = float(sd.get("decay", _update.decay))
        # rebind captured beta (via outer name)
        # we can’t reassign the free var directly; emulate by updating attribute
        _update.decay = beta_new
        # rebuild dicts
        total.clear();
        total.update({k: float(v) for k, v in sd.get("total", {}).items()})
        fix.clear();
        fix.update({k: float(v) for k, v in sd.get("fix", {}).items()})

    _update.state_dict = _state_dict
    _update.load_state_dict = _load_state_dict
    # -----------------------------------
    return _update
...
class Balancer:
    ...
    def state_dict(self) -> tp.Dict[str, tp.Any]:
        return {
            ...
            "per_batch_item": bool(self.per_batch_item),
            "total_norm": float(self.total_norm),
            "epsilon": float(self.epsilon),
            "rescale_grads": bool(self.rescale_grads),
            "monitor": bool(self.monitor),
            # store the averager’s EMA buffers + decay
            "averager": (self.averager.state_dict() if hasattr(self.averager, "state_dict") else None),
        }

    def load_state_dict(self, sd: tp.Dict[str, tp.Any]) -> None:
        self.weights = dict(sd["weights"])
        self.per_batch_item = bool(sd["per_batch_item"])
        self.total_norm = float(sd["total_norm"])
        self.epsilon = float(sd["epsilon"])
        self.rescale_grads = bool(sd["rescale_grads"])
        self.monitor = bool(sd.get("monitor", False))

        # rebuild averager with same decay and buffers
        self.averager = averager(sd.get("averager", {}).get("decay", 1.0))
        if sd.get("averager") and hasattr(self.averager, "load_state_dict"):
            self.averager.load_state_dict(sd["averager"])
